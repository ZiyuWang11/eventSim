...
void testLeNetPooling(bool debug) { 
...
void testLeNet(bool debug) { 
    ...
    LayerABC* layer_test[layerNum]; 
    layer_test[0] = new LayerConv(1, "Conv", devicePrecision, arraySizeX, arraySizeY, numADC, weight1, numK1, lutNum, af, sizeFM1*(sizeK1-1)+1+sizeK1/*bufferSize*/, depth1, sizeFM1, sizeK1, stride1, padding1);
    layer_test[1] = new LayerConv(2, "Conv", devicePrecision, arraySizeX, arraySizeY, numADC, weight, numK, lutNum, af, sizeFM*(sizeK-1)+1+sizeK/*bufferSize*/, depth, sizeFM, sizeK, stride, padding);
    layer_test[2] = new LayerConv(3, "Conv", devicePrecision, arraySizeX, arraySizeY, numADC, weight2, numK2, lutNum, af, sizeFM2*(sizeK2-1)+1+sizeK2/*bufferSize*/, depth2, sizeFM2, sizeK2, stride2, padding2);
    layer_test[3] = new LayerFC(3, "FC", devicePrecision, arraySizeX, arraySizeY, numADC, weight3, outNum3, lutNum, af);
    layer_test[4] = new LayerFC(4, "FC", devicePrecision, arraySizeX, arraySizeY, numADC, weight4, outNum4, lutNum, af);
    layer_test[5] = new LayerFC(5, "FC", devicePrecision, arraySizeX, arraySizeY, numADC, weight5, outNum5, lutNum, af);
    ...
    while (outCount < inputNum) {
        ...
        // if (clock > 1200000) {
        //     for (int i = 0; i < 3; ++i) {
        //         std::cout << "Clk " << clock << " Layer " << i + 1 << std::endl;
        //         layer_test[i]->checkBuffer();
        //     }
        // }
    }
